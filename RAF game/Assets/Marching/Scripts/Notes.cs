...
public class Notes : MonoBehaviour {
	...
	void Update () {
		...
		if ((Note1col.destroyNote1 == "Yes") && (gameObject.name == "note1(Clone)") && (transform.position.y <= -3.5)) {
			...
			Destroy (gameObject);
			Note1col.destroyNote1 = "NO"; 
		}

		if ((Note2Col.destroyNote2 == "Yes") && (gameObject.name == "note2(Clone)") && (transform.position.y <= -3.5)) {
			if (SongControl.multiplierScore >= 5) {
				SongControl.Score +=2; 
			}
			SongControl.Score +=1; 
			...
			Destroy (gameObject);
			Note2Col.destroyNote2 = "NO";
		}

		if ((Note3Col.destroyNote3 == "Yes") && (gameObject.name == "note3(Clone)") && (transform.position.y <= -3.5)) {
			if (SongControl.multiplierScore >= 5) {
				SongControl.Score +=2; 
			}
			SongControl.Score +=1; 
